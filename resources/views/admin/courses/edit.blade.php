        ...
        <form action="{{ route('admin.courses.update', $course->id) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                    class="w-full rounded-lg border-gray-300 focus:ring-indigo-500 focus:border-indigo-500">{{ old('description', $course->description) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="tools_count" min="0" required
                            value="{{ old('tools_count', $course->tools_count ?? 0) }}"
                            class="w-full pl-9 rounded-lg border-gray-300 focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
            ...
            <div class="flex justify-end gap-3 pt-6">
                <a href="{{ route('admin.courses.index') }}"
                    class="px-5 py-2.5 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition">Cancel</a>
                <button type="submit"
                    class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 rounded-lg text-sm font-medium text-white transition shadow-sm">
                    Save Changes
                </button>
            </div>
        </form>
